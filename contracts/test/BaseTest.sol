...
contract BaseTest is Test {
    ...
    function setUp() public {
        ...
        WrappedERC20Factory wrappedERC20Factory = new WrappedERC20Factory(IWrapped1155Factory(wrapped1155Factory));
        ...
        gnosisRouter = new GnosisRouter(IConditionalTokens(conditionalTokens), wrappedERC20Factory);
    }
    ...
    function assertOutcomesBalances(
        ...
        return assertOutcomesBalances(owner, conditionId, bytes32(0), partition, amount);
    }
    ...
    function assertOutcomesBalances(
        ...
        for (uint256 i = 0; i < partition.length; i++) {
            assertEq(
                IERC20(
                    gnosisRouter.getTokenAddress(IERC20(collateralToken), parentCollectionId, conditionId, partition[i])
                ).balanceOf(owner),
                amount
            );
        }
    ...
    function splitMergeAndRedeem(Market market, uint256[] memory partition, uint256 splitAmount) public {
        ...
        gnosisRouter.splitPosition(IERC20(collateralToken), bytes32(0), market.conditionId(), splitAmount);

        assertOutcomesBalances(msg.sender, market.conditionId(), partition, splitAmount);

        approveWrappedTokens(
            address(gnosisRouter), splitAmount, market.conditionId(), market.parentCollectionId(), partition
        );

        gnosisRouter.mergePositions(IERC20(collateralToken), bytes32(0), market.conditionId(), amountToMerge);

        assertOutcomesBalances(msg.sender, market.conditionId(), partition, amountToRedeem);

        gnosisRouter.redeemPositions(IERC20(collateralToken), bytes32(0), market.conditionId(), partition);
        ...
        assertOutcomesBalances(msg.sender, market.conditionId(), partition, splitAmountInSDai);
        ...
        assertOutcomesBalances(msg.sender, market.conditionId(), partition, amountToRedeemInSDai);
