...
| **[7-trading.md](7-trading.md)** | **Trading outcome tokens** via **Lens** (`lensQuoter` / `lensRouter`), which aggregates Uniswap V3 and Swapr (Algebra) on-chain. CoW Swap remains optional. Covers `fetchAmmQuote` / `AmmTrade`, collateral per chain, and execute helpers. |
