...
| **[7-trading.md](7-trading.md)** | **Trading outcome tokens** via **Lens smart quoter** (`lensQuoter`), which picks the best DEX and returns calldata for that DEX’s router. Covers `fetchAmmQuote` / `AmmTrade`, collateral per chain, and execute helpers. |
