...
- `userFromAddress` — build `DiscussionUser` from a wallet and an optional username
- `useDisplayName` — resolve a wallet's label (username > ENS > generated) plus its ENS name
- `resolveDisplayName` — the same precedence rule as a pure function, without the ENS lookup
- `addressUsername` — deterministic nickname generated from an address
