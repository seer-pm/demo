...
export function AddLiquidityV4Adapter({
  ...
  const outcomeIsToken0Resolved = outcomeTokenAddress.toLowerCase() === token0.toLowerCase();

  const token0Info = {
    address: token0,
    symbol: outcomeIsToken0Resolved ? (tokensInfo[0]?.symbol ?? "Outcome") : collateral.symbol,
  ...
  const token1Info = {
    address: token1,
    symbol: outcomeIsToken0Resolved ? collateral.symbol : (tokensInfo[0]?.symbol ?? "Outcome"),
    decimals: outcomeIsToken0Resolved ? collateral.decimals : 18,
    balance: outcomeIsToken0Resolved ? collateralBalance : outcomeBalance,
  };

  const computeDerived = (values: AddLiquidityFormValues, editedField: "amount0" | "amount1") => {
    const amount0 =
