import { Chain, gnosis as _gnosis, mainnet as _mainnet, hardhat, optimism, sepolia } from "wagmi/chains";
...
export { mainnet, gnosis, optimism, hardhat, sepolia };

export const SUPPORTED_CHAINS: Partial<
  Record<typeof gnosis.id | typeof mainnet.id | typeof optimism.id | typeof sepolia.id /*| typeof hardhat.id*/, Chain>
> =
  ...
  (
    SEER_ENV.VITE_IS_FAST_TESTNET === "1"
      ...
      : SEER_ENV.VITE_TESTNET_WEBSITE === "1"
        ? { [sepolia.id]: sepolia }
        : { [gnosis.id]: gnosis, [mainnet.id]: mainnet, [optimism.id]: optimism }
