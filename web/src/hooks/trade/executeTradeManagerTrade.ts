import { toastifyTx } from "@/lib/toastify";
import { config } from "@/wagmi";
import { sendTransaction } from "@wagmi/core";
import { Address, TransactionReceipt } from "viem";
import { Execution } from "../useCheck7702Support";
import { TradeManagerTrade } from "./TradeManagerTrade";

export async function executeTradeManagerTrade(
  trade: TradeManagerTrade,
  account: Address,
): Promise<TransactionReceipt> {
  const tradeExecution = await getTradeManagerTradeExecution(trade, account);

  const result = await toastifyTx(async () => sendTransaction(config, tradeExecution), {
    txSent: { title: "Executing trade..." },
    txSuccess: { title: "Trade executed!" },
  });

  if (!result.status) {
    throw result.error;
  }

  return result.receipt;
}

export async function getTradeManagerTradeExecution(trade: TradeManagerTrade, account: Address): Promise<Execution> {
  const populatedTransaction = await trade.swapTransaction({
    recipient: account,
  });

  return {
    to: populatedTransaction.to! as `0x${string}`,
    data: populatedTransaction.data!.toString() as `0x${string}`,
    value: BigInt(populatedTransaction.value?.toString() || 0),
    chainId: trade.chainId,
  };
}
