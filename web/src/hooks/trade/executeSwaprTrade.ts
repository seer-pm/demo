...
export async function executeSwaprTrade(
  ...
  if (isSellToNative) {
    ...
    return await multicallSellToNative(amountOut, populatedTransaction.data!.toString(), account);
  }
  const populatedTransaction = await trade.swapTransaction({
    recipient: account,
  });
...
export async function multicallSellToNative(amountOut: string, swapData: string, recipient: string) {
  const routerAddress = "0xffb643e73f280b97809a8b41f7232ab401a04ee1";
  ...
  const routerInterface = new ethers.utils.Interface(routerAbi);
  const unwrapData = routerInterface.encodeFunctionData("unwrapWNativeToken", [amountOut, recipient]);

  const contract = new ethers.Contract(routerAddress, routerInterface, signer);
  const result = await toastifyTx(
    () => contract.multicall([swapData, unwrapData]).then((result: Transaction) => result.hash),
    {
      txSent: { title: "Executing trade..." },
      txSuccess: { title: "Trade executed!" },
    },
