...
async function prepareMint(props: ProvideLiquidityProps, recipient: Address | undefined): Promise<PrepareMintResult> {
  ...
  for (let index = 0; index < formData.length; index++) {
    ...
    const initialPrice = buildPrice(token0, token1, Number(config.centerPrice));
    const priceLowerObj = buildPrice(token0, token1, Number(config.minPrice));
    const priceUpperObj = buildPrice(token0, token1, Number(config.maxPrice));
