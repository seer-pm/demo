...
export function handleSwapHelper(event: SwapEvent, subgraphConfig: SubgraphConfig = getSubgraphConfig()): void {
  ...
  // Swaps fire for all PoolManager pools; on networks with allowedHooks we only
  // create Bundle/PoolManager on Initialize of an allowed pool. Skip early.
  ...
  if (bundle === null || poolManager === null) {
    log.warning('Bundle or PoolManager not initialized yet, skipping swap for pool: {}', [poolId])
    return
  }
