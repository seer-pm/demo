...
export function handleModifyLiquidityHelper(
  ...
  // ModifyLiquidity fires for all pools; Bundle/PoolManager only exist after an
  // allowed Initialize. Check pool first, then globals, to avoid null aborts.
  ...
  if (bundle === null || poolManager === null) {
    log.debug('handleModifyLiquidityHelper: Bundle or PoolManager not found for pool {}', [poolId])
    return
  }
  ...
  if (token0 && token1) {
    ...
    updatePositionFromModifyLiquidity(event, pool, subgraphConfig)
...
/**
 * Tracks PositionManager NFT positions for indexed pools. The PositionManager passes
 * `bytes32(tokenId)` as the ModifyLiquidity salt, and mints the NFT (Transfer) before
 * modifying liquidity in the same transaction, so the owner is read from the contract
 * when the position is first seen. Later transfers are handled in `handleTransfer`.
 */
export function updatePositionFromModifyLiquidity(
  event: ModifyLiquidityEvent,
  pool: Pool,
  subgraphConfig: SubgraphConfig,
): void {
  const positionManagerAddress = subgraphConfig.positionManagerAddress
  if (positionManagerAddress.length == 0) {
    return
  }
  if (event.params.sender.toHexString().toLowerCase() != positionManagerAddress) {
    return
  }
  ...
  if (position === null) {
    ...
    const ownerResult = PositionManager.bind(Address.fromString(positionManagerAddress)).try_ownerOf(tokenId)
    if (ownerResult.reverted) {
      log.warning('updatePositionFromModifyLiquidity: ownerOf({}) reverted', [tokenId.toString()])
      position.owner = event.transaction.from.toHexString()
    } else {
      position.owner = ownerResult.value.toHexString()
    }
  }
