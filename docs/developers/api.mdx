...
Read `score` as a summary of the wallet's **cross-market** distribution. It is not a time-series metric: `lossBurn` is a terminal-state loss and a strict lower bound on true drawdown (it cannot see a position that was deep underwater mid-window and recovered), `breadth` measures concentration across markets rather than the smoothness of an equity curve, and `returns` has a capital denominator but no risk denominator. `scoreBreakdown.method` is `"markets"` and identifies which implementation produced the number.
