...
| `breakdown` | No | `1` or `true` to include `scoreBreakdown` on every row. |
...
Each row carries `score` (0-100, one decimal) and `tier` (`Elite` ≥85, `Great` ≥70, `Good` ≥55, `Average` ≥40, `Weak` below). It is a weighted mean of five clamped-linear components:

| Component | Weight | Measure | 0 points | 100 points |
|---|---|---|---|---|
| `returns` | 25% | ROI = `pnl_usd / capital_usd` | −25% | +75% |
| `profitFactor` | 25% | gross profit / gross loss, over markets | 0.5 | 3.0 |
| `hitRate` | 20% | share of markets ending in profit | 35% | 75% |
| `lossBurn` | 15% | gross loss / capital | 50% | 0% |
| `breadth` | 15% | `1 − bestMarket / grossProfit` | 0 (one market) | 0.7 |

`score` is `null`, never 0, when the wallet has fewer than 3 markets above $1 of capital at risk, or less than $100 of capital in the window. When it is null the row also carries `scoreUnavailable` — `{ reason: "markets" | "capital", scoredMarketCount, minScoredMarkets, minCapitalUsd }` — so a client can say which gate was missed. Note that `marketCount` counts every traded market while the gate counts only those above $1 of capital at risk, so a wallet can show a large `marketCount` and still be unscored. Two priors keep small books honest: the hit rate carries 5 pseudo-markets at 50%, and the profit factor carries symmetric prior mass of 5% of capital on both sides, so a book with no losing market reads a finite ratio rather than infinity.
