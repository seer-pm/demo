...
Covers the account **and the TradeExecutor contracts it owns**. Someone trading through an executor holds the outcome tokens inside that contract and the DEX indexes the swap with the executor as `recipient`, so querying the address alone returned only what the EOA did directly. Rows carry `sourceWallet`, which differs from the requested account when an executor produced them, and are deduped across the wallets by `eventId`. Executor discovery is best-effort: if it fails the response is the account's own rows, as before.
...
interface TransactionData {
  ...
  /** Source row id, unique per event. */
  eventId?: string;
  /** Wallet the row was indexed under. Absent means the requested account. */
...
Period P/L for an account (outcome MTM, including what the account holds in LP positions, + router collateral legs − swap/LP cashflow). Adding or removing liquidity on its own does not move P/L; price moves and fees compounded into the position do. Fees accrued but not collected are not counted. **Generic markets only** — Futarchy is excluded so non-primary notional (PNK/GNO, …) is not mixed into the profile’s primary units. `get-portfolio` still lists Futarchy positions.
...
| `search` | No | Case-insensitive substring filter on username or address. A leading `@` is optional. Only stored Seer usernames are matched: ENS names and generated display names are not searchable. Username matching needs at least 3 characters and resolves at most 500 wallets; a shorter fragment still filters by address. |
...
| `rankFor` | No | `0x`-prefixed wallet address. Returns `{ app, chainId, period, sort, dir, address, rank, total }` instead of leaderboard rows. |
...
Each row includes an optional `username` when the wallet has chosen one. Usernames are opt-in, so its absence means only that the wallet never set one.
...
Those statistics describe an **owner's** book, not an address's. A wallet that trades through a TradeExecutor contract holds its capital on one address and, after the contract sweeps the outcome tokens out, its value on another; each market would fall below the $1 gate on both. So the per-market books of an owner and its executors are combined at materialization, before the gate, and the resulting statistics are stored on the owner's row while the executors' rows carry zeros. `market_count` follows them: it counts *distinct* markets, so summing each member's own count double-counted every market an owner and its executor both traded. The additive totals — `pnl_usd`, `volume_usd`, `capital_deployed` — do stay per address and still sum across the merge.
...
| get-portfolio | GET | `account`, opt. `chainId`/`all`, `collateralProfile` | Wallet positions (filtered by profile); each row has `chainId` and `sourceWallet` |
| get-portfolio-value | GET | `account`, `chainId` or `all`, opt. `collateralProfile` | Portfolio value + 24h delta (USD) |
| get-portfolio-identity | GET | `account` | The account plus the TradeExecutor contracts it owns |
