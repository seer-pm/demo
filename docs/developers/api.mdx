...
| `search` | No | Case-insensitive substring filter on username or address. A leading `@` is optional. Only stored Seer usernames are matched — ENS names and generated display names are not searchable. |
