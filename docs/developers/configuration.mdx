...
title: "Configuration"
---
The integration examples in this folder use [viem](https://viem.sh) with a **public client** (read-only) and a **wallet client** (signed transactions). Create both once and reuse them across [Create a market](/developers/guides/create-a-market), [Resolve a market](/developers/guides/resolve-a-market), [Split, merge and redeem](/developers/guides/split-merge-and-redeem), [Conditional market](/developers/guides/conditional-markets), and [Futarchy markets](/developers/guides/futarchy-markets).

<Note>
The viem configuration below is provided as an example. If your project already has `publicClient` and `walletClient` (e.g. from Wagmi, RainbowKit or another library), you can use them directly; the examples in this documentation only need clients compatible with viem’s interface for `readContract`, `simulateContract`, `writeContract`, and `waitForTransactionReceipt`.
</Note>
...
The examples in [Create a market](/developers/guides/create-a-market), [Resolve a market](/developers/guides/resolve-a-market), and [Split, merge and redeem](/developers/guides/split-merge-and-redeem) use **@seer-pm/sdk**.
