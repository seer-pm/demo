...
Same idea as [Resolve a market](/developers/guides/resolve-a-market): the question must be answered and finalized on Reality.eth. Then call either:
...
We use the [Viem setup](/developers/configuration): `getPublicClient(chain)` and `getWalletClient(chain, process.env.PRIVATE_KEY)`. Use **@seer-pm/sdk** for addresses: `getFutarchyFactoryAddress(chainId)`, `getRouterAddress({ id: proposalAddress, type: "Futarchy", chainId })`. Futarchy contracts are deployed on **Gnosis** (chain 100).
