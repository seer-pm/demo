...
We use the [Viem setup](/developers/configuration) and **@seer-pm/sdk**. For split/merge/redeem on the conditional market use `getRouterAddress(market)` and the parent’s wrapped outcome token as collateral (see below). You need a **parent market** already created; the parent can be root or conditional.

### 1. Create a conditional categorical market

Use `getCreateMarketExecution` as in [Create a market](/developers/guides/create-a-market), with `parentMarket` and `parentOutcome` set to the parent market address and the outcome index this market is conditional on:
