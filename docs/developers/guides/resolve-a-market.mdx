...
---
To resolve a market, the user calls the `resolve` function on the `Market` contract.
...
Once the market is resolved, users can redeem winning position tokens for the underlying collateral via the `Router` contract (merge and redeem flow).
