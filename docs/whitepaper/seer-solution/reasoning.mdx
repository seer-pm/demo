...
Here we chose to use a modified (to allow partial bid) English auction, instead of other types of auctions such as sealed bid Vickery auction[^vickrey], for the following reasons:
...
* **Speed**: By not using sealed bids, we save ourselves from the extra delay introduced by commit and reveal schemes[^commitment-scheme].
...
[^vickrey]: [Vickrey auction](https://en.wikipedia.org/wiki/Vickrey_auction) — Wikipedia
[^commitment-scheme]: [Commitment scheme](https://en.wikipedia.org/wiki/Commitment_scheme) — Wikipedia
