...
A 0-100 rating of how a wallet traded the markets in a period, shown on the [leaderboard](/app/leaderboard). It is a weighted mean of five components — returns (25%), profit factor (30%), [hit rate edge](#hit-rate-edge) (15%), loss burn (15%) and breadth (15%) — so it reflects *how* a profit was made, not only how large it was. The mean is then pulled toward 50 by `n / (n + 10)`, where `n` is the number of markets counted, so a thin book reads undecided rather than excellent. Scores map to five tiers: Elite (85+), Great (70+), Good (55+), Average (40+) and Weak (below 40).

### Hit rate edge

How far a wallet's share of winning markets sits above the rate its own realised payoffs need to break even, in points of probability. One of the five Trader Score components. Because a prediction market pays out inversely to the price paid, a wallet whose winners average ten times its losers breaks even at about a 9% hit rate, while one trading near even money breaks even near 50% — so a bare hit rate is not comparable across strategies, and a fixed band marks down every long-shot strategy whether or not it is profitable.

### Loss burn

A wallet's total realised losses as a share of the capital it put at risk. One of the five Trader Score components. It is a terminal-state total, not a maximum drawdown: it measures where each market finished, so it cannot see a position that was deep underwater during the period and recovered.

### Breadth

Whether a wallet's profit came from many markets or from a single one. One of the five Trader Score components, computed as `1 − bestMarket / grossProfit`. It measures concentration across markets, not consistency over time.
